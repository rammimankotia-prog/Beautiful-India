...
const PilgrimageTourDetailView = () => {
    ...
    const cityPath = tour.tour_city_path ? tour.tour_city_path.split(',').map(c => c.trim()) : [];
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-slate-950 pb-32">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                        <div className="max-w-3xl space-y-4">
                            <span className="inline-block px-4 py-1.5 bg-orange-500 text-white text-[10px] font-black uppercase tracking-[3px] rounded-full shadow-lg">
                                {tour.tour_destination?.[0] || 'Spiritual Journey'}
                            </span>
                            <h1 className="text-5xl md:text-7xl font-serif font-black text-white leading-tight drop-shadow-xl">{tour.title}</h1>
                            {cityPath.length > 0 && (
            ...
            <main className="max-w-7xl mx-auto px-6 md:px-12 py-16 grid grid-cols-1 lg:grid-cols-3 gap-16 relative">
                ...
                <div className="lg:col-span-2 space-y-16">
                    ...
                    <section className="space-y-6">
                        <h2 className="text-3xl font-serif font-black text-slate-800 dark:text-white uppercase tracking-widest border-b-2 border-orange-500/20 pb-4 inline-block">The Journey</h2>
                        <div 
                    ...
                    {cityPath.length > 0 && (
                        <section className="space-y-8 bg-orange-50/50 dark:bg-orange-950/20 p-8 md:p-12 rounded-[40px] border border-orange-100 dark:border-orange-900/30">
                            <h2 className="text-2xl font-serif font-black text-slate-800 dark:text-white uppercase tracking-widest text-center">Path of Devotion</h2>
                            <div className="relative pt-6">
                                ...
                                <div className="flex flex-col md:flex-row justify-between relative z-10 gap-8 md:gap-4 overflow-x-auto pb-6 snap-x custom-scrollbar">
                    ...
                    {tour.tour_itinerary && tour.tour_itinerary.length > 0 && (
                        <section className="space-y-8">
                            <h2 className="text-3xl font-serif font-black text-slate-800 dark:text-white uppercase tracking-widest border-b-2 border-orange-500/20 pb-4 inline-block">Daily Itinerary</h2>
                            <div className="space-y-6 relative border-l-2 border-slate-200 dark:border-slate-800 ml-4 md:ml-6 pl-8 md:pl-12">
                                {tour.tour_itinerary.map((day, idx) => (
                                    <div key={idx} className="relative group cursor-pointer" onClick={() => setActiveDay(activeDay === idx ? null : idx)}>
                                        {/* Timeline Dot */}
                    ...
                    {/* Gallery Engine */}
                    {tour.tour_gallery && tour.tour_gallery.length > 0 && (
                        <section className="space-y-8 pb-12">
                            <h2 className="text-3xl font-serif font-black text-slate-800 dark:text-white uppercase tracking-widest border-b-2 border-orange-500/20 pb-4 inline-block">Sacred Sights</h2>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {tour.tour_gallery.map((img, idx) => (
                                    <div key={idx} className={`rounded-2xl overflow-hidden aspect-square ${idx === 0 ? 'col-span-2 row-span-2' : ''}`}>
                                        <img src={img} alt={`Gallery ${idx + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-700 cursor-pointer" />
                                    </div>
                                ))}
                            </div>
                        </section>
                ...
                {/* Sidebar Widget Area (Original Desktop Layout) */}
                        ...
                        <div className="bg-orange-50 dark:bg-orange-950/20 rounded-[32px] p-8 border border-orange-100 dark:border-orange-900/30">
                            ...
                            <ul className="space-y-4">
                                <li className="flex items-center gap-4 text-orange-900 dark:text-orange-200">
                                    <span className="material-symbols-outlined text-orange-500 bg-white dark:bg-slate-900 p-2 rounded-full shadow-sm">event</span>
                                    <div>
                                        <p className="text-[10px] uppercase font-bold text-orange-500/70">Dates</p>
                                        <p className="font-medium text-sm">
                                            {tour.tour_dates_ongoing === 'true' 
                                                ? 'Flexible Deparures' 
                                                : (tour.tour_dates_start ? `${new Date(tour.tour_dates_start).toLocaleDateString('en-GB')}` : 'TBA')}
                                        </p>
                                    </div>
                                </li>
                                {tour.tour_type && tour.tour_type.length > 0 && (
                                    <li className="flex items-center gap-4 text-orange-900 dark:text-orange-200">
                                        <span className="material-symbols-outlined text-orange-500 bg-white dark:bg-slate-900 p-2 rounded-full shadow-sm">self_improvement</span>
                                        <div>
            ...
            {/* Sticky Booking Bar (Mobile/Tablet OR Global anchored bottom) */}
