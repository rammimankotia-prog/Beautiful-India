import{r as i,j as e,L as o}from"./index-DyEvkPrc.js";const M=()=>{const[k,z]=i.useState([]),[T,S]=i.useState([]),[s,_]=i.useState("all"),[l,C]=i.useState(""),[h,f]=i.useState(!1),[d,u]=i.useState(1),[c,w]=i.useState(1),p=12,x=5;i.useEffect(()=>{fetch(`/data/guides.json?t=${Date.now()}`,{headers:{"Cache-Control":"no-cache",Pragma:"no-cache",Expires:"0"}}).then(t=>t.json()).then(t=>{const a=[...t].sort((r,n)=>{const Y=r.date?new Date(r.date):new Date(0);return(n.date?new Date(n.date):new Date(0))-Y});z(a),f(!0)}).catch(t=>{console.error("Error fetching guides:",t),f(!0)}),fetch(`/data/tours.json?t=${Date.now()}`).then(t=>t.json()).then(t=>{const a=t.filter(r=>r.status==="active");if(a.length>0){const r=[...a].sort(()=>.5-Math.random());S(r.slice(0,2))}}).catch(t=>console.error("Error fetching tours:",t))},[]);const P=[{id:"all",label:"All Guides",icon:"public"},{id:"destination",label:"Destinations",icon:"location_on"},{id:"tip",label:"Tips & Hacks",icon:"tips_and_updates"}],m=k.filter(t=>{const a=t.status!=="draft",r=s==="all"||t.type===s,n=t.title.toLowerCase().includes(l.toLowerCase())||t.description.toLowerCase().includes(l.toLowerCase());return a&&r&&n}),g=m.filter(t=>t.type==="destination"),b=m.filter(t=>t.type==="tip"),y=m.filter(t=>t.type==="blog"||!t.type),j=Math.ceil(g.length/p)||1,v=Math.ceil(b.length/x)||1,D=g.slice((d-1)*p,d*p),E=b.slice((c-1)*x,c*x),$=t=>{u(t),setTimeout(()=>{window.scrollTo({top:document.querySelector(".destination-section")?.offsetTop-100,behavior:"smooth"})},100)},G=t=>{w(t),setTimeout(()=>{window.scrollTo({top:document.querySelector(".tips-section")?.offsetTop-100,behavior:"smooth"})},100)},N=({current:t,total:a,onChange:r})=>e.jsxs("div",{className:"flex justify-center items-center gap-6 mt-8 mb-4",children:[e.jsx("button",{onClick:()=>r(Math.max(1,t-1)),disabled:t===1,className:"pagination-btn",children:e.jsx("span",{className:"material-symbols-outlined",children:"chevron_left"})}),e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx("span",{className:"text-xs font-bold text-slate-400 uppercase tracking-widest",children:"Page"}),e.jsx("span",{className:"text-lg font-black text-violet-600 leading-none",children:t}),e.jsxs("span",{className:"text-xs font-bold text-slate-400 uppercase tracking-widest",children:["of ",a]})]}),e.jsx("button",{onClick:()=>r(Math.min(a,t+1)),disabled:t===a,className:"pagination-btn",children:e.jsx("span",{className:"material-symbols-outlined",children:"chevron_right"})})]}),R=[{label:"Adventure",icon:"🏔️"},{label:"Honeymoon",icon:"💑"},{label:"Budget Travel",icon:"💰"},{label:"Gastronomy",icon:"🍜"},{label:"Europe",icon:"🏰"},{label:"Photography",icon:"📷"},{label:"Backpacking",icon:"🎒"},{label:"Beaches",icon:"🏖️"}];return e.jsxs("div",{"data-page":"travel_guides_category_landing",style:{fontFamily:"'Inter', 'Segoe UI', sans-serif"},children:[e.jsx("style",{children:`
